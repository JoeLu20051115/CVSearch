...
from .utils import *
from .modeling_dispatch import (
    finalize_option_losses,
    finalize_token_logits,
...
class ModelInternvl:
    def __init__(self, model_path: str, device: str = "cuda:0", torch_dtype=torch.bfloat16, **kwargs) -> None:
        ...
        self.use_flash_attn = transformers.utils.is_flash_attn_2_available()
    ...
    @contextmanager
    def cached_image_encoding(self):
        """Reuse one exact pixel tensor for this sample's frozen vision model."""
        model = self.model
        original = model.extract_feature
        had_override = "extract_feature" in vars(model)
        previous_override = vars(model).get("extract_feature")
        cached_pixels = cached_features = cached_key = None

        @wraps(original)
        def extract_feature(pixel_values):
            nonlocal cached_pixels, cached_features, cached_key
            if torch.is_grad_enabled() or any(
                module.training for module in (model, model.vision_model, model.mlp1)
            ):
                cached_pixels = cached_features = cached_key = None
                return original(pixel_values)
            device_type = pixel_values.device.type
            key = (
                pixel_values.shape, pixel_values.dtype, pixel_values.device,
                torch.is_autocast_enabled(device_type),
                torch.get_autocast_dtype(device_type),
                id(model.vision_model), id(model.mlp1), model.select_layer,
                model.downsample_ratio, model.ps_version,
            )
            if cached_key == key and torch.equal(pixel_values, cached_pixels):
                return cached_features.clone()
            features = original(pixel_values)
            cached_pixels = pixel_values.detach().clone()
            cached_features = features.detach().clone()
            cached_key = key
            return features
        ...
        try:
            ...
            if had_override:
                model.extract_feature = previous_override
            else:
                del model.extract_feature
            cached_pixels = cached_features = cached_key = None
    ...
    @torch.inference_mode()
    def label_token_losses(self, image_pil, question, codes, searched_nodes: List[Node] = None):
        token_ids = single_token_choice_ids(self.tokenizer, codes)
        image_list = self.process_nodes_to_image_list(searched_nodes, image_pil)
        image_input = [image_list[-1]] if len(image_list) > 1 else image_list
        pre_information = "pre_information_image2" if len(image_input) > 1 else "pre_information_image1"
        qs = self.prompts["search"][pre_information] + question
        prompt = self.get_prompt_from_qs(qs)
        full_pixel_values = []
        num_patches_list = []
        for index, image in enumerate(image_input):
            use_anyres = len(image_input) == 1 if index == 0 else True
            pixel_values = load_image(
                image, input_size=self.input_size[0], max_num=self.anyres_num,
        ...
        model_inputs = {key: value.to(self.device) for key, value in model_inputs.items()}
        outputs = self.model(**model_inputs)
        return finalize_token_logits(outputs.logits[0, -1], token_ids)
    ...
    def multiple_choices_with_losses(self, image_pil, question, options, searched_nodes: List[Node] = None):
        ...
        # InternVL replaces image-token embeddings in place, preserving sequence
        # length. Tokenization alone supplies the original option-loss boundary.
        len_question_input_ids = model_inputs.input_ids.shape[1]

        loss_list = []
        full_pixel_values = full_pixel_values.to(self.device)
        image_flags = torch.ones(full_pixel_values.shape[0], dtype=torch.long, device=self.device)
        lm = getattr(self.model, "language_model", None)
        forward_kwargs = {}
        if (type(self.model).__name__ == "InternVLChatModel"
                and type(lm).__name__ == "InternLM2ForCausalLM"
                and not self.model.training and not lm.training):
            # Pinned attention only retains/returns KV for this flag; option
            # scoring never consumes those states. Generation is unchanged.
        ...
        for option in options:
            ...
            full_model_inputs['image_flags'] = image_flags
            full_input_ids = full_model_inputs.input_ids

            output_option = self.model(**full_model_inputs, **forward_kwargs)
            logits = output_option.logits[:, len_question_input_ids - 1:-1]
            ...
            # The inference-mode scalar keeps no graph; release the full logits
            # and returned KV cache before allocating the next option's output.
            del output_option, logits
