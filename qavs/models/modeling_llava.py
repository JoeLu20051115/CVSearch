...
# LLaVA's QFormer imports these helpers from their pre-4.57 location.
from transformers import modeling_utils as _modeling_utils, pytorch_utils as _pytorch_utils
from transformers.utils import is_flash_attn_2_available
for _helper in ("apply_chunking_to_forward", "find_pruneable_heads_and_indices", "prune_linear_layer"):
    if not hasattr(_modeling_utils, _helper):
        setattr(_modeling_utils, _helper, getattr(_pytorch_utils, _helper))
...
class Model(ABC):
    def __init__(self, model_path: str, conv_type: str = "qwen_1_5", device: str = "cuda:0", torch_dtype=torch.float16,
                 attn_implementation=None, padding_side="left", **kwargs) -> None:
        attn_implementation = attn_implementation or ("flash_attention_2" if is_flash_attn_2_available() else "sdpa")
        disable_torch_init()
        ...
        checkpoint_config = json.loads((Path(model_path) / "config.json").read_text())
        language_model = (
            str(checkpoint_config.get("model_type", "")) + " "
            + " ".join(checkpoint_config.get("architectures", ()))
        ).casefold()
        if "qwen" in language_model:
    ...
    def label_token_losses(self, image_pil, question, codes, searched_nodes: List[Node] = None):
        ...
        image_list = self.process_nodes_to_image_list(searched_nodes, image_pil)
        prompt_tag = self.get_prompt_tag(image_list)
        prompt_values = self.prompts.get(prompt_tag, {})
        if "pre_information" in prompt_values:
            prefix = prompt_values["pre_information"]
        else:
            image_list = [image_list[-1]] if len(image_list) > 1 else image_list
            prefix_key = (
                "pre_information_image2" if len(image_list) > 1
                else "pre_information_image1"
            )
            prefix = self.prompts["search"][prefix_key]
        qs = prefix + question
        prompt = self.get_prompt_from_qs(qs)
        image_tensor, image_sizes = self.process_image_list_to_tensor(image_list)
        input_ids = tokenizer_image_token(
        ...
        forward_kwargs = {}
        if (
            isinstance(self.model, LlavaQwenForCausalLM)
            and not self.model.training
            and self.model.dtype in (torch.float16, torch.bfloat16)
            and "logits_to_keep" in signature(Qwen2ForCausalLM.forward).parameters
        ):
            # Project only the final token; keep the complete transformer sequence.
            forward_kwargs["logits_to_keep"] = 1
        outputs = self.model(
            input_ids, images=image_tensor, image_sizes=image_sizes,
            modalities=["image"], return_dict=True, **forward_kwargs,
        )
...
class ModelGlobalLocal(Model):
    ...
    def cached_image_encoding(self):
        """Reuse one exact pixel tensor within an inference sample only."""
        model = self.model
        original = model.encode_images
        had_override = "encode_images" in vars(model)
        previous_override = vars(model).get("encode_images")
        ...
        def encode_images(images):
            ...
            key = (
                images.shape, images.dtype, images.device,
                torch.is_autocast_enabled(device_type),
                torch.get_autocast_dtype(device_type),
            )
            if cached_key == key and torch.equal(images, cached_pixels):
                return cached_features.clone()
            features = original(images)
            cached_pixels = images.detach().clone()
        ...
        try:
            ...
            if had_override:
                model.encode_images = previous_override
            else:
                del model.encode_images
            cached_pixels = cached_features = cached_key = None
